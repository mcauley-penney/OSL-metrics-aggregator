"""Expose functionality for gathering metrics about the communicators in a repo's issues."""
...
def gather_all_issues_comm_metrics(issue_data: dict) -> dict:
    ...
    Returns:
        dict: {period str: dict of metrics from graph of "conversation" for period key}
    """
    ...
    for period, issue_nums in issue_buckets.items():
        ...
        for num in issue_nums:
            cur_bucket_graph = build_issue_conversation_matrix(
...
def build_issue_conversation_matrix(
    ...
    """
    # need list of items in current issue that we are connecting to,
    # then reset when issue changes
    cur_added_nodes: list = []

    # add OP
    # is first, so no need for idempotent add or adding edges
    userid: str = cur_issue["userid"]
    cur_vertex_obj = cur_graph.add_vertex(name=userid)
    cur_added_nodes.append(cur_vertex_obj)

    for _, comment in cur_issue["comments"].items():
        userid = comment["userid"]

        # add vertices idempotently, like in a set
        try:
        ...
        # We want to add connections only "during" the current issue.
        # We do not want to add connections that span issues, but, if
        # a node exists in multiple issues, that is no problem
        for existing_vertex_obj in cur_added_nodes:
            try:
                cur_graph.es.find(_between=([cur_vertex_obj], [existing_vertex_obj]))

            except ValueError:
                if cur_vertex_obj["name"] != existing_vertex_obj["name"]:
                    cur_graph.add_edge(cur_vertex_obj, existing_vertex_obj)
                    cur_graph[cur_vertex_obj, existing_vertex_obj] = 1
